...
app.post('/api/auth/forgot', async (req, res) => {
    ...
    try {
        const user = await prisma.user.findUnique({ where: { email } });
        if (!user)
            return res.status(200).json({ ok: true }); // не выдаём информацию о существовании
...
app.put('/api/admin/users/:id', authMiddleware, async (req, res) => {
    ...
    if (!email && typeof name === 'undefined')
...
app.get('/api/ctf/leaderboard', async (req, res) => {
    ...
    try {
        ...
        for (const s of solves) {
            ...
            if (!byUser[uid])
                byUser[uid] = { userId: s.userId, email: s.user.email, name: s.user.name || undefined, solves: 0, score: 0 };
            byUser[uid].solves += 1;
...
app.post('/api/ctf', authMiddleware, async (req, res) => {
    ...
    if (!title || !flag)
        return res.status(400).json({ error: 'title and flag required' });
    try {
...
app.post('/api/ctf/:id/submit', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (existing)
            return res.json({ ok: false, alreadySolved: true });
...
app.post('/api/ai/chat', authMiddleware, async (req, res) => {
    try {
        ...
        const answer = await ai_service_1.AIService.chat(messages);
